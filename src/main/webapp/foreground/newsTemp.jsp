			...
			<div class="datas">
				<ul>
					<c:forEach var="newestNews" items="${newestNewsList }">
						<li><a href="news.do?action=show&newsId=${newestNews.newsId }" title="${newestNews.title }">${fn:substring(newestNews.title,0,22) }</a></li>
					</c:forEach>
				</ul>
			</div>
		</div>
			...
			<div class="datas">
				<ul>
					<c:forEach var="hotNews" items="${hotNewsList }">
						<li><a href="news.do?action=show&newsId=${hotNews.newsId }" title="${hotNews.title }">${fn:substring(hotNews.title,0,22) }</a></li>
					</c:forEach>
				</ul>
			</div>
		</div>
