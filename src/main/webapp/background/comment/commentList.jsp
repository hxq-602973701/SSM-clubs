...
<script>
    ...
    function commentsDelete() {
        ...
        if (confirm("确认要删除这些评论吗？")) {
            $.post("commentDelete.do", {commentIds: commentIds},
                function (result) {
                    var result = eval('(' + result + ')');
                    if (result) {
                        alert("成功删除!");
                        window.location.href = "${pageContext.request.contextPath}/commentList.do";
                    } else {
                        alert("删除失败!");
                    }
                }
    ...
    function commentDelete(commentId) {
        if (confirm("确认要删除这条评论吗？")) {
            $.post("commentDelete.do", {commentIds: commentId},
                function (result) {
                    var result = eval('(' + result + ')');
                    if (result) {
                        alert("删除成功！");
    ...
    <div class="search_content" style="vertical-align: middle;padding-right: 20px;">
        ...
        <div style="float: right;">
            <form action="commentListA.do" method="get">
                评论日期：<input type="text" id="s_bCommentDate" name="s_bCommentDate" class="Wdate" onClick="WdatePicker()"
                            style="width: 100px;" value="${s_bCommentDate }"/>
                &nbsp;到&nbsp;<input type="text" id="s_aCommentDate" name="s_aCommentDate" class="Wdate"
                                    onClick="WdatePicker()" style="width: 100px;" value="${s_aCommentDate }"/>
        ...
        <table class="table table-hover table-bordered">
            ...
            <c:forEach var="commentBack" items="${commentBackList }" varStatus="status">
                <tr>
                    ...
                    <td><a title="${commentBack.newsTitle }" target="_blank"
                           href="news.do?newsId=${commentBack.newsId }">${fn:substring(commentBack.newsTitle,0,15) }...</a>
                    </td>
                    <td><fmt:formatDate value="${commentBack.commentDate }" type="date" pattern="yyyy-MM-dd"/></td>
                    <td>
                        <button class="btn btn-mini btn-danger" type="button"
                                onclick="commentDelete(${commentBack.commentId})">删除
                        </button>
                    </td>
                </tr>
            </c:forEach>
